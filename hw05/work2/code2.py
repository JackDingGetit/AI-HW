...
class LeNet5(nn.Module):
    ...
    结构：两个卷积层 -> 两个池化层 -> 三个全连接层
    """
    def __init__(self):
        """初始化网络结构，定义各层组件"""
        super(LeNet5, self).__init__()
        # 卷积层1：输入1通道(灰度图)，输出6通道，5x5卷积核
    ...
    def forward(self, x):
        ...
        """
        # 卷积层1 -> ReLU激活 -> 池化层1
        x = self.pool1(self.relu(self.conv1(x)))  # 输出: [batch_size, 6, 14, 14]
        # 卷积层2 -> ReLU激活 -> 池化层2
        x = self.pool2(self.relu(self.conv2(x)))  # 输出: [batch_size, 16, 5, 5]
        # 将特征图展平
        x = x.view(-1, 16 * 5 * 5)  # 输出: [batch_size, 400]
        # 全连接层1 -> ReLU激活
